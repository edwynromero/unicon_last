...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearLower = 1936;
	this.rangeYearUpper = 2037;
	this.minDate = new Date(1936,0,1);
...
Epoch.prototype.setLang = function()  //all language settings for Epoch are made here.  Check Date.dateFormat() for the Date object's language settings
{
	...
	this.months_sh = new Array('Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dec');
	this.monthup_title = 'Ir al mes proximo';
	this.monthdn_title = 'Ir al mes previo';
	this.clearbtn_caption = 'Limpiar';
	this.clearbtn_title = 'Borra cualquier fecha seleccionada';
	this.maxrange_caption = 'Este es el maximo rango';
};
//-----------------------------------------------------------------------------
...
Epoch.prototype.outputDate = function (vDate, vFormat) //PUBLIC: outputs a date in the appropriate format (DEPRECATED)
{
	...
	return vFormat.replace(/y{1,4}/g, vYear).replace(/mm/g, vMonth).replace(/dd/g, vDay).replace(/hh/g, vHour).replace(/nn/g, vMinute).replace(/ss/g, vSecond);
};
//-----------------------------------------------------------------------------
...
window.onload = function () {
						/* Estos son para el archivo ingEmp */
