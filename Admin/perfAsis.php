...
echo menu_vertical();
  echo panel();
 $per = $_GET['id_asistencia'];
          $conexion = conectar();

  $q1= "SELECT * FROM asistencias  WHERE id_asistencia=" .$per."";
...
$conexion = conectar();
$query_Recordset1 = "SELECT * FROM asistencias  WHERE id_asistencia=" .$per."";
...
<div class="datagrid">
<table>
<thead><h1>Lista de asistencias</h1></thead><span>

   FECHA <?php echo $row['fecha_asistencia'];  ?>   </span>
  <thead><tr>
  <th>Fecha</th><th>Asistencia</th><th>Personal</th><th>Maquina</th><th>Turno</th>
  ...
  <?php do { ?>
    <tr <?php echo $i++ % 2 ? 'class=""' : 'class="alt"'; ?>>
      ...
      <td class="active"><?php if($row_Recordset1['estado_asistencia'] == 2)
  echo "asistio";
  else
  echo "no asistio"; ?></td>
   <td><?php echo $row['id_personal_asistencia']; ?></td>
